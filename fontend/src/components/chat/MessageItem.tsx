...
import type { Conversation, Message, Participant } from "@/type/chat"
...
const MessageItem = ({
    ...
    const isShowTime = index === 0 || new Date(message.createdAt).getTime() - new Date(prev?.createdAt || 0).getTime() > 300000 // 5 phút
    ...
    return (
        <>
            {/* time */}
            {isShowTime && (
                <span className="text-sm text-center text-muted-foreground px-1">
            ...
            >
                ...
                <div className={cn("max-w-xs lg:max-w-md space-y-1 flex flex-col",
                    ...
                    <Card className={cn("p-3", message.isOwn ? "border-0" : "")}>
